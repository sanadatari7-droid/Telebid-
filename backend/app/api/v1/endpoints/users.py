...
async def create_user(body: UserCreate, conn=Depends(get_db), current_user=Depends(require_roles("ADMIN"))):
    ...
    await execute(conn,
        "INSERT INTO users (username,email,password_hash,full_name,dept_id,job_title,phone,created_by) VALUES ($1,$2,$3,$4,$5,$6,$7,$8)",
        body.username, body.email, pw_hash, body.full_name, body.dept_id, body.job_title, body.phone, current_user.user_id)
    ...
    try:
        await execute(conn, "INSERT INTO audit_logs (user_id,action,entity_type,entity_id,details) VALUES ($1,'USER_CREATED','user',$2,$3)",
            current_user.user_id, uid, f"Created user: {body.username}")
    except Exception:
        pass
...
async def get_user(user_id: int, conn=Depends(get_db), current_user=Depends(require_roles("ADMIN"))):
    user = await fetch_one(conn, """
        ...
        WHERE u.user_id=$1 GROUP BY u.user_id, d.dept_name""", user_id)
...
async def update_user(user_id: int, body: UserUpdate, conn=Depends(get_db), current_user=Depends(require_roles("ADMIN"))):
    ...
    args.append(user_id)
    await execute(conn, f"UPDATE users SET {','.join(updates)} WHERE user_id=${len(args)}", *args)
...
async def reset_password(user_id: int, body: PasswordReset, conn=Depends(get_db), current_user=Depends(require_roles("ADMIN"))):
    if len(body.new_password) < 8:
        raise HTTPException(status_code=400, detail="Password must be at least 8 characters")
    await execute(conn, "UPDATE users SET password_hash=$1, failed_attempts=0, is_locked=FALSE WHERE user_id=$2",
...
async def lock_user(user_id: int, conn=Depends(get_db), current_user=Depends(require_roles("ADMIN"))):
    await execute(conn, "UPDATE users SET is_locked=TRUE WHERE user_id=$1", user_id)
...
async def unlock_user(user_id: int, conn=Depends(get_db), current_user=Depends(require_roles("ADMIN"))):
    await execute(conn, "UPDATE users SET is_locked=FALSE, failed_attempts=0 WHERE user_id=$1", user_id)
...
async def deactivate_user(user_id: int, conn=Depends(get_db), current_user=Depends(require_roles("ADMIN"))):
    ...
    await execute(conn, "UPDATE users SET is_active=FALSE WHERE user_id=$1", user_id)
    return {"message": "Deactivated"}

@router.patch("/{user_id}/activate")
async def activate_user(user_id: int, conn=Depends(get_db), current_user=Depends(require_roles("ADMIN"))):
    await execute(conn, "UPDATE users SET is_active=TRUE WHERE user_id=$1", user_id)

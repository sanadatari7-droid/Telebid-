...
async def list_employees(employee_type:Optional[str]=None, conn=Depends(get_db), current_user=Depends(get_current_user)):
    sql = "SELECT e.* FROM employees e WHERE e.is_active=TRUE"
    args = []
    if employee_type:
...
async def create_employee(body:EmpCreate, conn=Depends(get_db), current_user=Depends(require_roles("ADMIN"))):
    await execute(conn,
        "INSERT INTO employees (user_id,employee_code,full_name,email,department,job_title,employee_type,tech_specialty) VALUES ($1,$2,$3,$4,$5,$6,$7,$8)",
        body.user_id, body.employee_code, body.full_name, body.email,
        body.department, body.job_title, body.employee_type, body.tech_specialty)
...
async def update_employee_profile(emp_id: int, body: EmpProfileUpdate,
    ...
    args.append(emp_id)
    await execute(conn, f"UPDATE employees SET {','.join(updates)} WHERE emp_id=${len(args)}", *args)
...
async def list_sectors(conn=Depends(get_db), current_user=Depends(get_current_user)):
    ...
    rows = await fetch_all(conn, "SELECT DISTINCT sectors_covered FROM employees WHERE sectors_covered IS NOT NULL AND sectors_covered != '' ORDER BY sectors_covered")
    all_sectors = set()
    for r in rows:

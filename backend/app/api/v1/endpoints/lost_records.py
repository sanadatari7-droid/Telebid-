...
async def create_lost(opp_id: int, body: LostCreate, conn=Depends(get_db), current_user=Depends(get_current_user)):
    ...
    await execute(conn, """
        ...
        body.could_revisit, body.revisit_notes, current_user.user_id)
    lost_id = await fetch_val(conn, "SELECT lost_id FROM lost_records WHERE lost_number=$1", lost_number)
    await execute(conn,
        "UPDATE opportunities_v2 SET status='LOST', phase='Dropped', lost_date=$1, loss_reason=$2, loss_type=$3, competitor_name=$4, winner_name=$5, winner_tcv=$6, updated_at=NOW(), updated_by=$7 WHERE opp_id=$8",
        body.lost_date, body.loss_reason, body.loss_type, body.competitor_name, body.winner_name, body.winner_tcv, current_user.user_id, opp_id)
    try:
        await execute(conn,
...
async def get_by_opp(opp_id: int, conn=Depends(get_db), current_user=Depends(get_current_user)):
    return await fetch_one(conn, "SELECT * FROM lost_records WHERE opp_id=$1 AND is_deleted=FALSE", opp_id)
...
async def get_lost(lost_id: int, conn=Depends(get_db), current_user=Depends(get_current_user)):
    lost = await fetch_one(conn, """
        ...
        LEFT JOIN users bm ON l.bid_manager_id=bm.user_id
        LEFT JOIN users lb ON l.lost_by=lb.user_id
        WHERE l.lost_id=$1 AND l.is_deleted=FALSE""", lost_id)
...
async def update_lost(lost_id: int, body: LostUpdate, conn=Depends(get_db), current_user=Depends(get_current_user)):
    ...
    args.append(lost_id)
    await execute(conn, f"UPDATE lost_records SET {','.join(updates)} WHERE lost_id=${len(args)}", *args)
